...
class HIPLib():
    ...
    def process_ip_sec_packet(self, packet):
        ...
        from the raw socket - now handles AH instead of ESP
        """
        logging.debug("Processing incoming AH packet");
        ...
        try:
            ...
            logging.debug("Received IPv4 packet with AH payload, size: %d bytes" % len(data))
            logging.debug("AH payload (first 50 bytes): %s" % data[:50].hex())

            ah_packet     = IPSec.AHPacket(data);
            ...
            logging.debug("Got AH packet from %s to %s of %d bytes" % (src_str, dst_str, len(data)))
            logging.debug("AH packet - SPI: 0x%08x, Seq: %d, Next Header: 0x%02x, Payload Len: %d" %
                (ah_packet.get_spi(), ah_packet.get_sequence(), ah_packet.get_next_header(), ah_packet.get_payload_length()))
            # Get SA record for AH processing
            ...
            logging.debug(hexlify(ihit))
            logging.debug(hexlify(rhit))
            logging.debug("------------------- HMAC key AH ------------------");
            logging.debug(hexlify(hmac_key));
            """

            # Extract ICV from AH packet
            received_icv = ah_packet.get_icv();
            logging.debug("Received ICV length: %d bytes" % len(received_icv))
            logging.debug("Received ICV: %s" % received_icv.hex())

            # Verify AH authentication
            auth_data = ah_packet.get_auth_data();
            calculated_icv = hmac_alg.digest(bytearray(auth_data));

            logging.debug("Auth data length: %d bytes" % len(auth_data))
            logging.debug("Calculated ICV: %s" % calculated_icv.hex())

            if received_icv != calculated_icv:
                logging.critical("Invalid ICV in AH packet - authentication failed!");
                logging.critical("Expected: %s" % calculated_icv.hex())
                logging.critical("Received: %s" % received_icv.hex())
                return  (None, None, None);

            logging.debug("AH authentication successful!")

            # Extract the unencrypted payload (this is the key difference from ESP)
            frame = ah_packet.get_payload();
            logging.debug("Extracted payload length: %d bytes" % len(frame))
            logging.debug("Payload (first 50 bytes): %s" % frame[:50].hex() if len(frame) > 0 else "empty")
            ...
            logging.critical("Exception occured. Dropping AH packet.");
            logging.critical(e);
            traceback.print_exc();
    ...
    def process_l2_frame(self, frame, ihit, rhit, src_str):
        try:
            ...
            if hip_state.is_unassociated() or hip_state.is_closing() or hip_state.is_closed():
                ...
                # Get SA record and construct the AH payload (replacing ESP)
                try:
                ...
                # Create AH packet instead of ESP
                # AH provides authentication but no encryption - inner packets remain visible
                logging.debug("Creating AH packet for data plane")
                logging.debug("Data length: %d bytes" % len(data))
                logging.debug("Data (first 50 bytes): %s" % data[:50].hex() if len(data) > 0 else "empty")

                ah_packet = IPSec.AHPacket(next_header=0x00);  # 0x00 = No next header (for L2 frame tunneling)
                ah_packet.set_spi(spi);
                ah_packet.set_sequence(seq);

                logging.debug("AH header created - SPI: 0x%08x, Seq: %d" % (spi, seq))

                # Add the unencrypted payload directly (this makes it visible in Wireshark)
                ah_packet.add_payload(data);

                # Pre-allocate ICV space for proper authentication calculation
                dummy_icv = b'\x00' * hmac_alg.LENGTH
                ah_packet.add_icv(dummy_icv)

                # Calculate HMAC over AH header (with ICV field zeroed) + payload
                auth_data = ah_packet.get_auth_data();
                icv = hmac_alg.digest(bytearray(auth_data));

                logging.debug("Auth data length: %d bytes" % len(auth_data))
                logging.debug("Calculated ICV: %s" % icv.hex())

                # Replace dummy ICV with real ICV
                payload = ah_packet.get_payload()
                ah_packet.buffer = ah_packet.buffer[:IPSec.AH_FIXED_HEADER_LENGTH]
                ah_packet.add_icv(icv)
                ah_packet.add_payload(payload)

                logging.debug("AH packet created successfully")
                logging.debug("Final AH packet size: %d bytes" % len(ah_packet.get_byte_buffer()))
                logging.debug("AH packet (first 50 bytes): %s" % ah_packet.get_byte_buffer()[:50].hex())

                # Send AH packet to destination
                ipv4_packet = IPv4.IPv4Packet();
                ipv4_packet.set_version(IPv4.IPV4_VERSION);
                ipv4_packet.set_destination_address(dst);
                ipv4_packet.set_source_address(src);
                ipv4_packet.set_ttl(IPv4.IPV4_DEFAULT_TTL);
                ipv4_packet.set_protocol(IPSec.IPSEC_PROTOCOL);  # Now using AH protocol (0x33)
                ipv4_packet.set_ihl(IPv4.IPV4_IHL_NO_OPTIONS);
                ipv4_packet.set_payload(ah_packet.get_byte_buffer());

                logging.debug("Sending AH packet to %s, total IPv4 packet size: %d bytes" %
                    (Utils.ipv4_bytes_to_string(dst), len(ipv4_packet.get_buffer())))
